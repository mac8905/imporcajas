...
class CotizacionDescargarPDF extends CI_Controller
{
...
//Cabecera de página
   public function index()
   {
	...
	$obtenerDatos = $this->cotizacion_model->pdf($cotizacion_id);
	foreach($obtenerDatos as $row){
		$this->pdf->Cell(45,7,utf8_decode('Cotización Nº'),0,0,'R');
		$this->pdf->Ln();
		$this->pdf->Cell(162);
		$this->pdf->Cell(23,7,utf8_decode($row->cotizacion_id),0,0,'R');
		break;
	}
	...
	foreach($obtenerDatos as $row){
		...
		$this->pdf->Cell(50,5,utf8_decode($row->cotizacion_fecha),1,0,'C');//Fecha de creación
		$this->pdf->Ln();
		$this->pdf->SetFont('Arial','B',10);
		...
		$this->pdf->SetFont('Arial','',9);
		$this->pdf->Cell(50,5,utf8_decode($row->cotizacion_fechavencimiento),1,0,'C');//Fecha de vencimiento
		break;
    }
   //Tabla simple
		...
		$this->pdf->Ln();
		foreach($obtenerDatos as $row){
			$this->pdf->Cell(130,5,utf8_decode("Observaciones: ".($row->cotizacion_observacion)),0,0,'L');
			break;
		}
		$this->pdf->SetFont('Arial','',10);
		...
		$this->pdf->Output("cotización.pdf", 'D');
