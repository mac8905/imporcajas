...
class Proveedor extends CI_Controller{
	...
	public function consultar(){
		if (!$this->session->userdata('correo')) {
			...
			$obtenerId = $this->proveedor_model->mostrar($relacion_id);
			$data = array(
				'titulo' => 'Proveedores',
				'contenido' => 'backend/proveedor_mostrar_view',
	...
	/*Función para mostrar en un datagrid los datos de la tabla relación
	y realizar las operaciones básicas de crear, consultar, modificar y eliminar*/
	public function datagrid(){
		...
		#########################################################################
		#				CONFIGURACIONES DE LA TABLA PROVEEDOR					#
		#########################################################################
			$config['base_url'] = base_url()."proveedor/datagrid/";				#
			$config['total_rows'] = $this->proveedor_model->getCantidad();		#
			$config['per_page'] = '10';											#
			$config['num_links'] = '1';											#
		...
		#########################################################################
			...
			$data1 = array(
				'titulo' => 'Proveedores',
				'contenido' => 'backend/proveedor_datagrid_view',
				'consulta_proveedor_datagrid' => $this->proveedor_model->getContactos($config['per_page'],$this->uri->segment(3))
			);
	...
	public function eliminar(){
		...
		$this->proveedor_model->eliminar($id);
		redirect('proveedor/datagrid');
	...
	public function guardar() {
		...
		if($this->form_validation->run() == FALSE){
			...
			$this->proveedor_model->guardar($proveedor);
			$this->proveedor_model->guardarTelefono($this->getTelefono(), $proveedor);
			$this->proveedor_model->guardar_perfil($proveedor, $perfil);
			redirect('proveedor/datagrid');
		}
	...
	public function index(){
		if (!$this->session->userdata('correo')) {
			...
			$data = array(
				'titulo' => 'Proveedor',
				'contenido' => 'backend/proveedor_view',
	...
	#Método que retorna un registro de la tabla relacion para ser modificados
	public function modificar(){
		if (!$this->session->userdata('correo')) {
			...
			$relacion_id = $this->uri->segment(3);
			$obtenerProveedor = $this->proveedor_model->obtenerDatos($relacion_id);
				
			if($obtenerProveedor != FALSE){
				foreach($obtenerProveedor->result() as $row){
					if ($obtenerProveedor->num_fields()==12) {
				...
				$data3 = array(
					...
					'titulo' => 'Proveedores',
					'contenido' => 'backend/proveedor_modificar_view',
				);
	...
	//
	//Método que modifica o actualiza los datos de la tabla relación
	public function modificarEnlace() {
		...
		$this->proveedor_model->modificarProveedor($id, $proveedor);
		$this->proveedor_model->modificarPerfil($id, $perfiles);
		$this->proveedor_model->modificarTelefono($telefonos, $id);
		redirect('proveedor/datagrid');
